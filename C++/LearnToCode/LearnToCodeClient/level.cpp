...
void Level::Draw()
{
    ...
    for(auto& item : spikes){
        ...
        if (character->getBody()->GetPosition().x < item->getBody()->GetPosition().x + 25 &&
                character->getBody()->GetPosition().x > item->getBody()->GetPosition().x - 25)
            isGreen = true;
        else if (character->getBody()->GetPosition().x < item->getBody()->GetPosition().x - 25 &&
                     character->getBody()->GetPosition().x > item->getBody()->GetPosition().x - 75)
            isGreen = true;
...
void Level8::FillItemsVector()
{
    ...
    items.push_back(new WorldItemTuple(8,1,"spikes"));
    currentX = ReturnXCoordinate(2);
    currentY = ReturnYCoordinate(1);
}
