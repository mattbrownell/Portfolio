...
void MainWindow::switchToHomeMode()
{
    ...
    if(model->thisUser->isInstructor == false)
        ui->studentStats->hide();
    else
        ui->studentStats->show();
}
...
void MainWindow::connectEverything()
{
    ...
    connect(ui->loginOffline, SIGNAL(clicked(bool)), this, SLOT(offlineLoginClicked()));
    ...
    connect(ui->studentStats, SIGNAL(clicked(bool)), this, SLOT(studentStatsClicked()));
...
void MainWindow::offlineLoginClicked()
{
    ui->loginFailedLabel->hide();
...
void MainWindow::showStats()
{
    ...
    ui->highscoreLabel->hide();

    if (model->thisUser->isConnected)
    {
        model->saveProgress(currLevel, score);

        // Set highscore
        int highscore = model->thisUser->pointsPerLevel.at(currLevel - 1);
        ui->highschoolValue->setText(QString::number(highscore));
        ui->highscoreLabel->show();
    }
...
void MainWindow::studentStatsClicked()
{
    QUrl url = model->serverUrl + "/?showStat=" + model->thisUser->username;
