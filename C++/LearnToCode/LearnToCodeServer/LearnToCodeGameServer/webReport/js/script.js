...
function deleteUser(teacher, user) {
    ...
    if (confirm(errorMessage)) {
        ...
        $("#" + user.toLowerCase()).css("display", "none");
    }
}
...
function generateTable(data) {
    var html = '<tr><th></th><th>Username</th><th>Name</th><th>Current Level</th><th>Total Score</th></tr>';
    if (typeof(data[0]) === 'undefined') {
        return null;
    }
    if (data[0].constructor === Object) {
        for (var row in data) {
            html += '<tr data-username="' + data[row].id + '" id="' + (data[row].id).toLowerCase() + '" class="' + (data[row].id).toLowerCase() + " " + (data[row].fullName).toLowerCase() + " " + data[row].score + " " + (data[row].currentlevel).toLowerCase() + '">\r\n';
